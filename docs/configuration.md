...
| `attribution` | `true` | Ask the model to end every commit message and pull-request body it writes with `🤖 Generated with [wizolt](https://wizolt.readthedocs.io)`. `/set runtime.attribution off` stops it for the session, `false` for good |
